...
struct RingView: View {
    @Binding var transactions: [TransactionDisplayItem]
    
    private func ratio(for categoryIndex: Int) -> Double {
        transactions.sumOfTransactionsFilteredByCategory(Category.allCases[categoryIndex])
        /
        transactions.sumOfAmounts()
    }
    
    private func offset(for categoryIndex: Int) -> Double {
        (0..<categoryIndex).map(ratio).reduce(0, +)
    }
